...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val adapter = EpisodeAdapter { item -> startActivity(Intent(this, DetailActivity::class.java).putExtra("item_id", item.id)) }
    ...
    private fun refreshList() {
        lifecycleScope.launch {
            val items = app.database.dao().getAllEpisodes()
            (binding.recyclerView.adapter as EpisodeAdapter).items = items
            (binding.recyclerView.adapter as EpisodeAdapter).notifyDataSetChanged()
